...
export async function POST(req: Request) {
  try {
    ...
    if (!session.user.organization_id) {
      return new NextResponse("No organization associated with user", { status: 400 });
    }

    // Create default evidence types for the organization
    const types = await Promise.all(
      defaultEvidenceTypes.map(type =>
        prisma.evidenceType.create({
          data: {
            ...type,
    ...
    console.error("[EVIDENCE_TYPES_SEED]", error);
